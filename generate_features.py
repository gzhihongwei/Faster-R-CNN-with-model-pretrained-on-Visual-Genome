...
# Example:
# python generate_featues.py --net res101 --dataset vg --out test.csv --cuda
...
def parse_args():
    ...
    parser.add_argument(
        "--output_dir", help="output directory", default=None, type=Path
    )
    parser.add_argument(
        "--cfg",
        dest="cfg_file",
    ...
    parser.add_argument(
        "--split", help="Split to preprocess", default="train2014", type=str
    )
        ...
        with open("/data/visualgenome/image_data.json") as f:
            for item in json.load(f):
                ...
                filepath = os.path.join(
                    "/data/visualgenome/", item["url"].split("rak248/")[-1]
                )
                split.append((filepath, image_id))
    else:
        print("Unknown split")
...
def get_detections_from_im(
    ...
    return: dict of {'image_name', 'image_h', 'image_w', 'num_boxes', 'boxes', 'features'}
    ...
    x = (box_dets[:, 0:1] + box_dets[:, 2:3]) / 2
    y = (box_dets[:, 1:2] + box_dets[:, 3:4]) / 2
    w = box_dets[:, 2:3] - box_dets[:, 0:1]
    h = box_dets[:, 3:4] - box_dets[:, 1:2]

    width = np.size(im, 0)
    height = np.size(im, 1)

    # Normalize and concatenate
    reformed_boxes = np.concatenate(
        (x / width, y / height, w / width, h / height), axis=1
    )

    return {
        "visual_feats": pooled_feat[keep_boxes].cpu().detach().numpy(),
        "visual_pos": reformed_boxes,
    }
